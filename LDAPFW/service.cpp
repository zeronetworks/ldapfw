...
void printServiceState()
{
	...
	case SERVICE_STOPPED:
		outputMessage(L"\tLDAP Firewall Service stopped.");
		break;
	case SERVICE_START_PENDING:
		outputMessage(L"\tLDAP Firewall Service start pending.");
		break;
	case SERVICE_STOP_PENDING:
		outputMessage(L"\tLDAP Firewall Service stop pending.");
		break;
	case SERVICE_RUNNING:
		outputMessage(L"\tLDAP Firewall Service running.");
		break;
	case SERVICE_CONTINUE_PENDING:
		outputMessage(L"\tLDAP Firewall Service continue pending.");
		break;
	case SERVICE_PAUSE_PENDING:
		outputMessage(L"\tLDAP Firewall Service pause pending.");
		break;
	case SERVICE_PAUSED:
		outputMessage(L"\tLDAP Firewall Service paused.");
		break;
	default:
		outputMessage(L"\tLDAP Firewall Service status check failed!");
		break;
	}
}
...
void serviceInstall(DWORD startType)
{
	...
	{
		SERVICE_DESCRIPTION description = { const_cast<LPWSTR>(SERVICE_NAME) };
		ChangeServiceConfig2(schService.h, SERVICE_CONFIG_DESCRIPTION, &description);

		outputMessage(L"Service installed successfully");
	}
